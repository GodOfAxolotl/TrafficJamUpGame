class GraphicsLoader {
  ...
  void drawImage(int id, int x, int y, float scale) {
    if (id < 9000 && id != 4000) {
      if (id != 3) {  
        image(pics[id], x * scale, y * scale, pics[id].width*2, pics[id].height*2);  
      } else {   //Ticket 3: Cursorversetzung, der Cursor ragt über andere Tiles Hinaus
